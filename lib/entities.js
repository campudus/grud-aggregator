...
function getEntitiesOfTable(tableName) {
  ...
  var disableFollow = _options$disableFollo === undefined ? [] : _options$disableFollo;
  var pimUrl = options.pimUrl;
  var _options$maxEntriesPe = options.maxEntriesPerRequest;
  var maxEntriesPerRequest = _options$maxEntriesPe === undefined ? 500 : _options$maxEntriesPe;
  ...
  function getTableAndLinkedTablesAsPromise(tableId, disableFollow, maxEntriesPerRequest) {
    if (!promises[tableId]) {
      var promiseOfLinkedTables = (0, _pimApi.getCompleteTable)(pimUrl, tableId, maxEntriesPerRequest).then(function (table) {
