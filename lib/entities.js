...
function getEntitiesOfTable(tableNameOrNames) {
  ...
  function isColumnDisabled(columnName, disableFollow) {
    var disabledColumns = _lodash.default.reduce(disableFollow, function (disabledColumns, columns) {
      var head = _lodash.default.head(columns);

      var second = _lodash.default.nth(columns, 1);

      if (columns.length === 1) {
        return _lodash.default.concat(disabledColumns, head);
      }

      if (columns.length === 2 && head === "**") {
        return _lodash.default.concat(disabledColumns, second);
      }
