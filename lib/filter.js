...
function filter() {
  ...
  var _ref$excludeBacklinks = _ref.excludeBacklinks;
  var excludeBacklinks = _ref$excludeBacklinks === undefined ? false : _ref$excludeBacklinks;
  var _ref$path = _ref.path;
  var path = _ref$path === undefined ? [] : _ref$path;
  var _ref$predicate = _ref.predicate;
  var predicate = _ref$predicate === undefined ? defaultPredicate : _ref$predicate;
  var _ref$ignoreMissing = _ref.ignoreMissing;
  var ignoreMissing = _ref$ignoreMissing === undefined ? false : _ref$ignoreMissing;
    ...
    if (!_lodash2.default.isEmpty(path)) {
      var _ret = function () {
        ...
        if (firstTable) {
          ...
          addDependenciesOfTable(data, accTables, accTables[firstTable.id], excludeBacklinks, '' + firstTable.id, ignoreMissing);

          if (excludeBacklinks) {
            return {
              v: _lodash2.default.omitBy(removeBrokenLinksToFirstTable(accTables, firstTable.id), function (table) {
...
function addDependenciesOfTable(allTables, accTables, currentTable, excludeBacklinks, excludedTableId, ignoreMissing) {
  ...
  _lodash2.default.forEach(filteredMissing, function (linksInTable, tableId) {
    if (_lodash2.default.isNil(accTables[tableId])) {
      ...
      addDependenciesOfTable(allTables, accTables, accTables[tableId], excludeBacklinks, excludedTableId, ignoreMissing);
    }
  });
}
...
function matches(allTables, _ref2, row, currentTable, predicate) {
  ...
  var currentColumn = _ref3[0];

  var columnsToWalk = _ref3.slice(1);
