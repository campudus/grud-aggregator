            ...
            rows[row.id] = {
              ...
              values: _.flatMap(columns, function (column, index) {
                ...
                if (kind === "concat") {
                  ...
                  return [_.map(rowValue, function (subValue, subIndex) {
                    var subColumn = column.groups[subIndex];
                    if (subColumn.kind === "link") {
                      return _.map(subValue, function (cell) {
                        return cell.id;
                      });
                    } else if (subColumn.multilanguage && subColumn.languageType === "language") {
                      var value = subValue[defaultLanguage];
                      if (needsFallback(subColumn.kind, value)) {
                        var fallbackLangTag = _.find(fallbackLangTags, function (langTag) {
                          return !needsFallback(subColumn.kind, subValue[langTag]);
                        });
