...
function getDisplayName(tableOrColumn, langTag, fallbackLangTags) {
  ...
  if (!_lodash2.default.isNil(displayName)) {
    return displayName;
  }

  var fallbackLangTag = _lodash2.default.find(fallbackLangTags, function (langTag) {
    return !_lodash2.default.isNil(displayNames[langTag]);
  });
...
function tablesToLanguages(langtags) {
  ...
  if (fallbackOnly && _lodash2.default.some(langtags, function (value) {
  ...
  return function (tablesObj) {
    return _lodash2.default.reduce(langtags, function (acc, fallbackLangTags, language) {
      return Object.assign(acc, _defineProperty({}, language, _lodash2.default.mapValues(tablesObj, function (table) {
        ...
        return {
          ...
          columns: _lodash2.default.flatMap(columns, function (column) {
            ...
            return [_lodash2.default.omit(updatedColumn, ["toColumn", "multilanguage"])];
          }),
          rows: _lodash2.default.transform(rows, function (rows, row) {

            rows[row.id] = {
              final: row.final || false,
              values: _lodash2.default.flatMap(columns, function (column, index) {
                ...
                if (kind === "concat") {
                  ...
                  return [_lodash2.default.flatMap(rowValue, function (cell) {
                    if (!_lodash2.default.isEmpty(cell.internalName[defaultLanguage])) {
                  ...
                  return [_lodash2.default.map(rowValue, function (cell) {
                    return cell.id;
                  })];
                  ...
                  if (_lodash2.default.isNil(value)) {
                    var fallbackLangTag = _lodash2.default.find(fallbackLangTags, function (langTag) {
                      return !_lodash2.default.isNil(rowValue[langTag]);
                    });
