...
function tablesToLanguages(langtags) {
  return function (tablesObj) {
    return _lodash2.default.reduce(langtags, function (acc, fallbacks, language) {
            ...
            rows[row.id] = {
              values: _lodash2.default.flatMap(table.columns, function (c, index) {
                if (c.kind === 'concat') {
                  ...
                  var value = row.values[index][language];
                  if (_lodash2.default.isNil(value)) {
                    var fallbackValues = _lodash2.default.map(fallbacks, function (lang) {
                      return row.values[index][lang];
                    });
                    var valueIndex = _lodash2.default.findIndex(fallbackValues, function (v) {
                      return !_lodash2.default.isNil(v);
                    });
                    if (valueIndex >= 0) {
