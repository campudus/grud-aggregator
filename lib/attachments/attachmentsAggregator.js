  ...
  return _lodash2.default.reduce(chunks, function (promise, attachmentsInChunk) {
    return promise.then(function (currentProgress) {
      return Promise.all(_lodash2.default.map(attachmentsInChunk, function (item) {
        ...
        var currentInfo = database.get('attachments').defaultsDeep(_defineProperty({}, item.path, { id: item.path, downloaded: false, thumbnailed: false, minified: false })).get(item.path).value();
        ...
        return Promise.resolve().then(function () {
          ...
          return downloader.then(function () {
            ...
            return database.get('attachments').get(item.path).assign({ downloaded: true }).value();
          });
        }).catch(function (err) {
          ...
          return thumbnailer.then(function () {
            return database.get('attachments').get(item.path).assign({ thumbnailed: true }).value();
          });
        }).catch(function (err) {
          ...
          return minifier.then(function () {
            return database.get('attachments').get(item.path).assign({ minified: true }).value();
          });
        }).catch(function (err) {
          errorProgress({ message: 'Could not minify ' + item.path, error: err });
          return Promise.reject(err);
        }).catch(function () {
          database.get('attachments').get(item.path).assign({ id: item.path, downloaded: false, thumbnailed: false, minified: false }).value();
