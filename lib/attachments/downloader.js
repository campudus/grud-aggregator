...
function downloader() {
  ...
  return function (attachments) {
    return Promise.all(_lodash2.default.map(attachments, function (a) {
      if (_lodash2.default.isEmpty(a.url) || _lodash2.default.isEmpty(a.path)) {
        return Promise.reject(new Error('Expected array of {url, path} mappings.'));
      } else {
        var from = '' + pimUrl + a.url;
        var to = downloadPath + '/' + a.path;
        if (database.find(_path2.default.basename(to), 'downloaded')) {
          return Promise.resolve(to);
        } else {
          return download(from, to).then(function (to) {
