...
export function tablesToLanguages(
  langtags,
  { fallbackOnly = false, fallbackOnEmptyString = true } = {}
) {
                    ...
                    values: _.flatMap(columns, (column, index) => {
                      ...
                      if (kind === "concat") {
                          ...
                          _.flatMap(rowValue, (cell) => {
                            if (!_.isEmpty(cell.internalName[defaultLanguage])) {
                              ...
                              return {
                                ...
                                description:
                                  cell.description[defaultLanguage] || cell.description[fallback],
                                externalName:
                                  cell.externalName[defaultLanguage] || cell.externalName[fallback],
                        ...
                        if (needsFallback(kind, value)) {
                          const fallbackLangTag = _.find(
                            fallbackLangTags,
                            (langTag) => !needsFallback(kind, rowValue[langTag])
                          );
