...
export function filter(
  ...
  if (excludeBacklinks && filterBacklinks) {
    console.warn(
      "Both filterBacklinks and excludeBacklinks options are turned on. excludeBacklinks will override " +
  ...
  return (data) => {
    if (!_.isEmpty(path)) {
      ...
      if (firstTable) {
        ...
        accTables[firstTable.id].rows = _.transform(
          firstTable.rows,
          (rows, row) => {
        ...
        if (excludeBacklinks || filterBacklinks) {
          return _.omitBy(removeBrokenLinksToFirstTable(accTables, firstTable.id), (table) =>
            _.isEmpty(table.rows)
          );
        }
      }
...
function removeBrokenLinksToFirstTable(accTables, firstTableId) {
  return _.mapValues(accTables, (table) => {
    const columnIdxsOfLinks = _.transform(
      table.columns,
      (cols, column, idx) => {
...
function addDependenciesOfTable(
  ...
  const linksOfCurrentTableToCheck = _.transform(
    currentTable.columns,
    (acc, column, idx) => {
      if (column.kind === "link") {
        acc[column.toTable] = _.reduce(
          currentTable.rows,
          (linkIds, rowValue) => {
            if (_.isNil(rowValue)) {
              return linkIds;
              ...
              return _.union(
                linkIds,
                _.map(rowValue.values[idx], (link) => link.id)
              );
            }
          },
          acc[column.toTable] || []
  ...
  const missing = _.transform(
    linksOfCurrentTableToCheck,
    (missing, linkedRowIds, tableId) => {
      const isExcludedLink = excludeBacklinks && tableId === excludedTableId;
      if (!isExcludedLink) {
        const existingRows = _.isNil(accTables[tableId])
          ? []
          : _.map(accTables[tableId].rows, (row) => row.id);
  ...
  _.forEach(filteredMissing, (linksInTable, tableId) => {
    ...
    if (_.isNil(accTables[tableId])) {
      ...
      accTables[tableId].rows = _.transform(
        linksInTable,
        (rows, toRowId) => {
...
function matches(allTables, [, ...columnsToWalk], row, currentTable, predicate) {
  if (_.isEmpty(columnsToWalk)) {
    ...
    if (nextColumnIdx === -1) {
      ...
      return _.some(nextRows, (nextRow) =>
        matches(allTables, nextColumnsToWalk, nextRow, nextTable, predicate)
      );
    }
  }
}
