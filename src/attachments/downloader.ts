...
export function downloader({
  ...
  if (!_.isNil(progress) && !_.isFunction(progress)) {
    throw new Error(
      "Option `progress` needs to be a function ({error, message, currentStep, steps})."
    );
  }
          ...
          if (_.isEmpty(attachment.url) || _.isEmpty(attachment.path)) {
            ...
            if (database.find(path.basename(to), "downloaded")) {
              ...
              return download(from, to, headers)
                ...
                .catch((err) => {
                  if (errorImage) {
                    return copyFile(errorImage, to).then(() => {
                      if (progress) {
                        progress({
                          error: true,
                          message: `Downloaded ${to} from ${from}`,
      ...
      .get(
        ...
        (response) => {
          if (response.statusCode === 200) {
            ...
            reject(
              new Error(`Download of ${url} failed with status code ${response.statusCode}.\n`)
            );
          }
        }
      )
