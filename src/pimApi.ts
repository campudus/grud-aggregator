...
export function getTablesByNames(options, ...names) {
  return getAllTables(getOptionsFromParam(options, "getTablesByNames")).then((tables) =>
    _.filter(tables, (t) => _.some(names, (name) => t.name === name))
  );
}
...
export function getCompleteTable(options, tableId, maxEntries) {
  ...
  return request("GET", `${pimUrl}/tables/${tableId}`, { headers })
    ...
    .then((tableAndColumns) => {
      return request("GET", `${pimUrl}/tables/${tableId}/rows?offset=0&limit=${maxEntries}`, {
        headers,
      }).then((result) => {
        ...
        return requests.reduce(
          (promise, requestUrl) => {
            return promise.then((tableColumnsAndRows) => {
              return request("GET", requestUrl, { headers }).then((rowResult) => ({
                ...tableColumnsAndRows,
...
function getOptionsFromParam(options, fnName) {
  if (_.isString(options)) {
    console.warn(
      `${fnName}(pimUrl:string, ...) is deprecated. Use ${fnName}(options:object, ...) with an options object like {pimUrl: "http://..."}`
    );
    ...
    throw new Error(
      `Error in calling ${fnName}. Expected string or object but was ${typeof options}`
    );
  }
}
