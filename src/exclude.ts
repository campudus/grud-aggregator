...
export function exclude(options: any = {}) {
    ...
    const result = transform(
      ...
      (allTables, table) => {
        if (!includes(tablesToKickIds, table.id)) {
          const columnNamesToRemove = map(
            filter(paths, (p) => p[0] === table.name),
            (p) => p[1]
          );
  ...
  function removeColumns(table, columnNamesToRemove, removeLinksToTableIds) {
    const columnsToKickIndices = transform(
      table.columns,
      (ids, column, idx) => {
        const shouldKickByFunction = predicate(column, table);
        const shouldKickColumn = includes(columnNamesToRemove, column.name);
        const shouldKickLink =
          column.kind === "link" && includes(removeLinksToTableIds, column.toTable);
    ...
    const preservedColumns = filter(
      table.columns,
      (column, idx) => !includes(columnsToKickIndices, idx)
    );
