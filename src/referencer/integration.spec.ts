  ...
  it("works right after tablesToLanguages", () => {
    return Promise.resolve(allTables)
      ...
      .then((referenced) => {
        ...
        expect(de.testTable["1"].someLink[0].identifier).to.eql(
          "my identifying text, linking to row 1 in anotherTestTable"
        );

        expect(de.testTable["2"].someLink[0].identifier).to.eql(
          "my third row identifying text, linking to both rows in anotherTestTable"
        );

        expect(de.testTable["2"].someLink[1].identifier).to.eql(
